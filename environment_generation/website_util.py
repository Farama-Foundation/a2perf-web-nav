...
class Website(object):
    ...
    def __init__(self, first_page=None, design=None):
        ...
        assert (first_page is None) != (design is None), 'Either first_page or design must be provided, but not both.'
...
if __name__ == '__main__':
    gin.parse_config_files_and_bindings(
        ['/web_nav/gwob/configs/envdesign.gin'], None
    )

    numbers = range(0, 41)

    d2 = {
        'number_of_pages': len(numbers),
        'action_page': numbers,
        'action': numbers,
    }
    ...
    d = {
        ...
        'action_page': [0, 0, 0, 0],
        'action': [
            web_primitives.CONCEPTS.index(x)
            for x in ('deck', 'deck', 'deck', 'deck')
        ],
    }
